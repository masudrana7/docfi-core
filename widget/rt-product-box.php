...
* By : Radius Theme
*/
Class DocfiTheme_Product_Box extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			'classname' => 'rt-product-box',
			'description' => esc_html__( 'Product Display Widget' , 'docfi-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'rt-product-box', esc_html__( 'Docfi : Product display Box' , 'docfi-core' ), $widget_ops );		
	}
	public function widget( $args, $instance ) {
		...
		$title = ( !empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Product' , 'docfi-core' );
						...
						<?php if ( has_post_thumbnail() ){ ?>
							<a class="post-img-holder img-opacity-hover" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php
									if ( has_post_thumbnail() ){
										the_post_thumbnail( 'docfi-size4', ['class' => 'media-object'] );
									} else {
										if ( $show_no_preview_img == 'view' ) { ?>
											<img class="rt-lazy" src="<?php echo esc_url( DOCFI_ASSETS_URL ); ?>img/noimage_375X300.jpg" alt="<?php the_title_attribute(); ?>">
									<?php }
									}
								?></a>
						<?php } ?>
				...
				<div class="topic-box">				
					<?php if ( $show_post_image == 'yes' ) { ?>
						...
						<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>" class="rt-wid-post-img img-opacity-hover">
							<?php
								if ( has_post_thumbnail() ){
									the_post_thumbnail( 'docfi-size3', ['class' => 'media-object'] );
								} else {
									if ( $show_no_preview_img == 'view' ) { ?>
										<img class="rt-lazy" src="<?php echo esc_url( DOCFI_ASSETS_URL ); ?>img/noimage_375X300.jpg" alt="<?php the_title_attribute(); ?>">
								<?php }
	...
	public function form( $instance ) {
		...
		?>
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'number' )); ?>"><?php esc_html_e( 'Number of Product to show:', 'docfi-core' ); ?></label>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_price' )); ?>"><?php esc_html_e( 'Display product price?' , 'docfi-core'); ?></label></p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_cat' )); ?>"><?php esc_html_e( 'Display product category?', 'docfi-core' ); ?></label></p>	
