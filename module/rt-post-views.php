...
if( ! class_exists( 'RT_POST_VIEWS' )){
	...
	class RT_POST_VIEWS{
		...
		function _set_post_views(){
			...
			if( !DocfiTheme::$options['post_view'] || ! is_single() ){
				return;
			}
			...
			/*if( !DocfiTheme::$options['post_view'] || $page > 1  ){
				return false;
			}*/
		...
		function _posts_column_views( $defaults ){
			...
			if( DocfiTheme::$options['post_view'] ){
				$defaults['post_view'] = esc_html__( 'Views', 'docfi-core' );
			}
...
/*----------------------------------------------------------*/
if( !function_exists( 'docfi_views' )){

	function docfi_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$docfi_view_html = esc_html__( 'View' , 'docfi-core' );
		} else {
			$docfi_view_html = esc_html__( 'Views' , 'docfi-core' );
		}
		
		$docfi_views_html = $docfi_view_html . '<span class="view-number" >' . ' ' . $view_count . '</span> ';

		return '<span class="meta-views meta-item '. $views_class .'">'. $docfi_views_html.'</span> ';
	}
}
