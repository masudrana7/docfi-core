...
/* Post reading time */
if( !function_exists( 'docfi_reading_time' )){

	function docfi_reading_time(){
		...
		if( $reading_time < 1){
			$result = esc_html__( 'Less than a minute', 'docfi-core' );
		}
		elseif( $reading_time > 60 ){
			$result = sprintf( esc_html__( '%s hours read' ), floor( $reading_time / 60 ) );
		}
		else if ( $reading_time == 1 ){
			$result = esc_html__( '1 minute read', 'docfi-core' );
		} else {
			$result = sprintf( esc_html__( '%s minutes read', 'docfi-core' ), $reading_time );
		}
