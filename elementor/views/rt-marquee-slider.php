...
if( $data['rt_image']['url'] ) {
	$docfi_bg = 'url(' . $data['rt_image']['url'] . ')' ;
} else {
	$docfi_img_url = DOCFI_ASSETS_URL . 'img/marque_bg.jpg';
	$docfi_bg = 'url(' . $docfi_img_url . ')';
}
...
<div class="rt-marquee-slider marquee-slider-1">
	<div class="rt-marquee <?php echo esc_attr( $data['marquee_direction'] );?>">
		<div class="rt-marquee-item">
			...
			<h3 class="entry-title"><a href="<?php echo esc_url( $marquee_slides['url'] );?>" target="_blank" style="background-image: <?php echo esc_html( $docfi_bg ); ?>"><?php echo wp_kses_post( $marquee_slides['title'] ); ?></a></h3>
			<?php endforeach; ?> 
		</div>
		<div class="rt-marquee-item">
			<?php foreach ( $data['marquee_slide'] as $marquee_slides ): ?>
...
<div class="rt-marquee-slider marquee-slider-2" style="background-image: <?php echo esc_html( $docfi_bg ); ?>">
	<div class="rt-marquee <?php echo esc_attr( $data['marquee_direction'] );?>">
		<div class="rt-marquee-item">
			<?php foreach ( $data['marquee_slide'] as $marquee_slides ): ?>
